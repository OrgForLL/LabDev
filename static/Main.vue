<template>
  ...
  >
    <div v-loading="loading">
      ...
      <!--详情页-->
      <div v-if="colVisible">
        <Detail v-bind:sphhIn="this.mdata.sphh"></Detail>
      </div>
    </div>

    <!-- <div v-for="item in mdata.mlbgList" :key="item.Docid">
      <FlowLog v-if="item.show"  @goback="mlbgGoback(item)" v-bind:parIn="item"> </FlowLog>
    </div>
...
import Detail from "@/components/ProcessControl/Detail.vue";
import FlowLog from "@/components/Utils/FlowLog.vue";
...
export default {
  ...
  methods: {
    init() {
      // console.log(this.$status.userInfo)
      llApp.init();
