  ...
  <div id="ProcessControl">
    <div v-loading="loading">
      <div v-show="!colVisible">
        <h1 style="text-align: center">产品质量记录信息</h1>
        <el-form
          :model="mdata"
          label-width="70px"
        ...
        >
          <el-form-item style="margin-right: 40px" label="货号:">
          ...
          >
            <el-card class="box-card">
              <div slot="header" class="clearfix">
                ...
                <div>日期：{{ item.Zdrq }}</div>
                <div>操作人：{{ item.Zdr }}</div>
                </span>
                <el-button type="primary" @click="choiceImg(item.Id)"
                  v-show="(item.Id==0?false:true)" >上传</el-button                  
                >
                <el-button type="primary" @click="saveImg(item.Id)"
                  >{{(item.Id==0?"新增":"修改")}}</el-button                  
                >
              </div>
              <el-image
      ...
      <div id="floatDiv" v-show="colVisible">
        <el-button type="primary" @click="colVisible = false">返回</el-button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    uploadImg2(e) {
      ...
      // console.log(this.mdata.lxDataList.currentMydjid)
          ...
          reader.onload =  (evt)=> {
            ...
            resetOrientation(base64, orientation, (resultBase64) => {
              ...
              b64toBlob(resultBase64, (blob) => {
                ...
                // param.append("groupid", this.mdata.lxDataList.groupid);
                // param.append("tableid", this.mdata.lxDataList.currentMydjid);
                //调用接口上传图片
                return registerFace(param, config).then(
                  (res) => {
                    if (res == "ok") {
                      this.$message(file.name + "上传成功"); // 上传成功逻辑
                      this.searchImg(this.mdata.lxDataList.currentMydjid);
                    }
                  },
                  (err) => {
                    console.log(err);
      ...
      let registerFace =(param, config) => {     
        return new Promise( (resolve, reject)=> {
          this.$axiosPost
            .post(APIUTLFile + "?serviceGotoUrl="+
            encodeURIComponent(NetUrlUpload + "service/HttpRequestSkill.ashx?groupid="
            +this.mdata.lxDataList.groupid+"&tableid="
            +this.mdata.lxDataList.currentMydjid), param)
            .then((response) => {
    ...
    createChdmListHtml(obj, scData) {
      if(scData.CpzlgzList && scData.CpzlgzList.length>0){
          ...
          var addObj={};addObj.Id=0;addObj.imgList=[];
          var day2 = new Date();
          day2.setTime(day2.getTime());
          addObj.Zdrq = day2.getFullYear()+"-" + (day2.getMonth()+1) + "-" + day2.getDate();
          addObj.Zdr="";
    ...
    uploadDefined(file,  callback) {
      ...
      xhr.open("POST", APIUTLFile + "?serviceGotoUrl="+
         encodeURIComponent(NetUrlUpload + "service/HttpRequestSkill.ashx?groupid="
         +this.mdata.lxDataList.groupid+"&tableid="
